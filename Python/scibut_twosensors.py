...
"""
scibut_twosensors.py 
Reads eight 8bit messages from the Serial port (see below).
...
OUTPUT:
Text file containing (in columns 1 to 4)
1. Arduino timestamp (ms)
2. Photodiode reading
3. FSR reading
4. Python timestamp (secs)
...
Bytes 6 and 7 = Low and high byte of the FSR reading 
...
def report_package(r,dumpfile,time_now):
    ...
    b3 = ord(r[4])+256*ord(r[5]) # FSR
    b4 = r[6]

    #print b1,b2,b3 # for debugging
    output = "%i %i %i %f\n"%(b1,b2,b3,time_now) # preset row
